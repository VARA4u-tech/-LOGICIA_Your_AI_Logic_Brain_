...
import {
  Send, Trash2, Plus, ChevronDown, ChevronUp,
  Copy, Check, BarChart2, CheckCircle2, Lightbulb,
  ChevronRight, Sparkles, ArrowLeft,
  Bot, User as UserIcon, Hash, PanelLeftClose, PanelLeftOpen,
} from "lucide-react";
import {
...
/* ═══════════════════════════════════════════════════════════════
   TYPES
═══════════════════════════════════════════════════════════════ */
interface Step { label: string; math: string; explanation?: string; }
interface PlotData { x: number; y: number; }
interface SolutionData { method?: string; steps: Step[]; finalAnswer: string; graphData?: PlotData[]; }
interface Message { id: number; role: "user" | "ai"; content: string; solution?: SolutionData; timestamp: Date; }
...
const solveMath = (input: string): { content: string; solution?: SolutionData } => {
  ...
  if (trimmed.includes("2+2") || trimmed.match(/^2\s*\+\s*2$/)) {
    return { content: "Here's the solution to your arithmetic problem:", solution: { method: "Basic Arithmetic", steps: [{ label: "Identify operation", math: "2 + 2", explanation: "Simple addition of two integers" }, { label: "Compute sum", math: "2 + 2 = 4" }], finalAnswer: "4" } };
  }

  if (trimmed.includes("derivative") || trimmed.includes("d/dx")) {
    const graphData: PlotData[] = [];
    for (let x = -3; x <= 3; x += 0.5) graphData.push({ x, y: 2 * x });
    return { content: "I'll apply differentiation rules step by step:", solution: { method: "Power Rule — Differentiation", steps: [{ label: "Identify the function", math: "f(x) = x²", explanation: "A polynomial of degree 2" }, { label: "Apply the power rule", math: "d/dx [xⁿ] = n · xⁿ⁻¹" }, { label: "Final result", math: "d/dx [x²] = 2x" }], finalAnswer: "f'(x) = 2x", graphData } };
  }

  if (trimmed.includes("integral") || trimmed.includes("∫")) {
    const graphData: PlotData[] = [];
    for (let x = -3; x <= 3; x += 0.5) graphData.push({ x, y: (x * x) / 2 });
    return { content: "Applying integration rules:", solution: { method: "Power Rule — Integration", steps: [{ label: "Identify the integrand", math: "∫ x dx" }, { label: "Apply the power rule", math: "∫ xⁿ dx = (xⁿ⁺¹)/(n+1) + C" }, { label: "Evaluate", math: "∫ x¹ dx = x²/2 + C" }], finalAnswer: "x²/2 + C", graphData } };
  }
  ...
  if (trimmed.includes("quadratic") || trimmed.includes("x²") || trimmed.includes("x^2")) {
    ...
    const graphData: PlotData[] = [];
    const center = -b / (2 * a);
    for (let x = center - 5; x <= center + 5; x += 0.5) graphData.push({ x, y: a * x * x + b * x + c });
    return { content: `Let me solve the quadratic equation: **${a}x² + (${b})x + (${c}) = 0**`, solution: { method: "Quadratic Formula", steps: [{ label: "Coefficients", math: `a = ${a}, b = ${b}, c = ${c}` }, { label: "Discriminant", math: `Δ = b² − 4ac = ${disc}`, explanation: disc >= 0 ? "Positive/Zero → Real Roots" : "Negative → Complex Roots" }, { label: "Quadratic formula", math: `x = (−b ± √Δ) / 2a` }, { label: "Final roots", math: `x₁ = ${x1.toFixed(2)}, x₂ = ${x2.toFixed(2)}` }], finalAnswer: `x₁ = ${x1.toFixed(2)}, x₂ = ${x2.toFixed(2)}`, graphData } };
  }
  ...
  try {
    ...
    if (sanitized.length > 0 && input.match(/[0-9]/)) {
      ...
      if (typeof result === "number" && isFinite(result)) {
        return { content: "I computed that expression for you:", solution: { method: "Arithmetic Evaluation", steps: [{ label: "Expression", math: `${input.trim()} = ${result}` }], finalAnswer: `${result}` } };
      }
    }
  } catch { /* empty */ }

  return { content: "I'm ready to solve any math problem! Here are some examples you can try:\n\n• **Quadratic:** x^2 - 5x + 6 = 0\n• **Calculus:** derivative of x²\n• **Calculus:** integral of x\n• **Arithmetic:** 125 * 4 + 18\n\nJust type your problem and I'll walk you through it step by step." };
};
...
/* ═══════════════════════════════════════════════════════════════
   SOLUTION PANEL
═══════════════════════════════════════════════════════════════ */
const SolutionPanel = ({ steps, finalAnswer, method, graphData }: {
  steps: Step[]; finalAnswer: string; method?: string; graphData?: PlotData[];
}) => {
  ...
  return (
    <div className="mt-4 space-y-3 border-t border-border/40 pt-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        >
          {expanded ? <><span>HIDE</span><ChevronUp size={11} /></> : <><span>SHOW STEPS</span><ChevronDown size={11} /></>}
        </button>
      </div>
          ...
          <div className="h-[160px] sm:h-[200px] p-2 sm:p-3">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={graphData}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(120 100% 54% / 0.08)" />
                <XAxis dataKey="x" stroke="hsl(120 20% 40%)" fontSize={9} tickFormatter={(v) => v.toFixed(1)} tickLine={false} axisLine={false} />
                <YAxis stroke="hsl(120 20% 40%)" fontSize={9} tickFormatter={(v) => v.toFixed(1)} tickLine={false} axisLine={false} width={32} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(120 20% 6% / 0.95)", borderColor: "hsl(120 100% 54% / 0.3)", borderRadius: "10px", fontSize: "11px" }} itemStyle={{ color: "hsl(120 100% 54%)" }} cursor={{ stroke: "hsl(120 100% 54% / 0.4)", strokeWidth: 1.5 }} />
                <Line type="monotone" dataKey="y" stroke="hsl(120 100% 54%)" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: "hsl(120 100% 54%)", stroke: "white", strokeWidth: 2 }} animationDuration={1200} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {/* Steps */}
      <div className="overflow-hidden transition-all duration-500" style={{ maxHeight: expanded ? `${steps.length * 200 + 200}px` : "0px", opacity: expanded ? 1 : 0 }}>
        <div className="space-y-2.5 py-1">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-2.5 sm:gap-3 items-start">
              <div className="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 rounded-full border border-primary/30 bg-primary/5 flex items-center justify-center text-[9px] sm:text-[10px] font-display text-primary mt-0.5">{i + 1}</div>
              <div className="flex-1 min-w-0 overflow-hidden">
                <p className="text-[10px] text-muted-foreground tracking-wide uppercase font-display mb-1">{step.label}</p>
                <MathBlock expr={step.math} />
                {step.explanation && (
                  <p className="text-[10px] sm:text-[11px] text-muted-foreground leading-relaxed mt-1">
                    <ChevronRight size={9} className="inline mr-1 text-primary" />{step.explanation}
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="p-3 sm:p-4 rounded-xl border border-primary/40 bg-primary/8 flex items-center justify-between gap-2 sm:gap-3 flex-wrap">
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <p className="font-display text-[9px] tracking-[0.25em] text-primary/60 mb-0.5 uppercase">Final Answer</p>
            <p className="font-mono text-base sm:text-xl text-primary font-bold break-all" style={{ textShadow: "0 0 20px hsl(120 100% 54% / 0.6)" }}>{finalAnswer}</p>
          </div>
        </div>
  ...
  <div className="flex gap-3 sm:gap-4 items-start max-w-3xl mx-auto px-3 sm:px-6 py-3 animate-fade-in-up">
    ...
    <div className="flex items-center gap-1.5 sm:gap-2 pt-2">
      {[0, 0.15, 0.3].map((delay, i) => (
        <span key={i} className="w-2 h-2 rounded-full bg-primary/60" style={{ animation: `pulse 1.4s ease-in-out ${delay}s infinite` }} />
      ))}
      <span className="text-xs text-muted-foreground ml-1 font-display tracking-wider">Computing...</span>
    </div>
  </div>
);
...
const MessageBubble = ({ msg }: { msg: Message }) => {
  ...
  return (
    <div className={`group flex gap-2.5 sm:gap-4 max-w-3xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3 animate-fade-in-up ${isUser ? "flex-row-reverse" : ""}`}>
      {/* Avatar — hidden on very small phones to save space */}
      <div className={`hidden xs:flex flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full border items-center justify-center self-start mt-1 ${
        isUser ? "bg-primary/20 border-primary/40" : "bg-primary/10 border-primary/30"
      }`}>
        {isUser ? <UserIcon size={13} className="text-primary" /> : <Bot size={13} className="text-primary" />}
      </div>

      {/* Content */}
      <div className={`flex-1 min-w-0 ${isUser ? "flex flex-col items-end" : ""}`}>
        {/* Label + time */}
        <div className={`flex items-center gap-2 mb-1 sm:mb-1.5 ${isUser ? "flex-row-reverse" : ""}`}>
          <span className="font-display text-[10px] tracking-wider text-muted-foreground">{isUser ? "YOU" : "LOGICIA AI"}</span>
        ...
        {/* Bubble */}
        <div className={`rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap overflow-hidden ${
          isUser
          ...
          {msg.solution && (
            <SolutionPanel steps={msg.solution.steps} finalAnswer={msg.solution.finalAnswer} method={msg.solution.method} graphData={msg.solution.graphData} />
          )}
        </div>
      </div>
    </div>
...
const EmptyState = ({ onPrompt }: { onPrompt: (p: string) => void }) => (
  <div className="flex flex-col items-center justify-center h-full px-4 py-10 sm:py-16 text-center">
    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border border-primary/30 bg-primary/5 flex items-center justify-center mb-4 sm:mb-5" style={{ boxShadow: "0 0 30px hsl(120 100% 54% / 0.15)" }}>
      <Bot size={24} className="text-primary sm:hidden" />
      <Bot size={28} className="text-primary hidden sm:block" />
    </div>
    <h2 className="font-display text-lg sm:text-xl md:text-2xl font-bold text-primary mb-2 tracking-wider leading-tight" style={{ textShadow: "0 0 20px hsl(120 100% 54% / 0.5)" }}>
    ...
    <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 w-full max-w-xs xs:max-w-sm sm:max-w-md">
      {QUICK_PROMPTS.map((qp) => (
        <button key={qp.prompt} onClick={() => onPrompt(qp.prompt)}
          ...
          <Hash size={11} className="inline mr-2 text-primary/50 group-hover:text-primary transition-colors flex-shrink-0" />
...
const titleFromMessage = (text: string) => text.slice(0, 40) + (text.length > 40 ? "…" : "");
...
const Chat = () => {
  ...
  useEffect(() => {
    try { localStorage.setItem("logicia_sidebar_open", sidebarOpen.toString()); } catch { /* ignore */ }
  }, [sidebarOpen]);
  ...
  const newConversation = useCallback(() => {
    const conv: Conversation = { id: makeId(), title: "New Conversation", messages: [], createdAt: new Date() };
  ...
  const sendMessage = useCallback((text: string) => {
    ...
    let convId = activeId;
    if (!convId) {
      const conv: Conversation = { id: makeId(), title: titleFromMessage(trimmed), messages: [], createdAt: new Date() };
      setConversations(prev => [conv, ...prev]);
      convId = conv.id;
      setActiveId(conv.id);
    }
    ...
    setConversations(prev => prev.map(c => {
      ...
      return { ...c, messages: [...c.messages, userMsg], title: c.messages.length === 0 ? titleFromMessage(trimmed) : c.title };
    }));
    setInput("");
    setIsTyping(true);
    ...
    setTimeout(() => {
      ...
      const aiMsg: Message = { id: Date.now() + 1, role: "ai", content, solution, timestamp: new Date() };
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-background text-foreground">
        ...
        <div className="flex flex-col h-full w-64 sm:w-72 min-w-[16rem] sm:min-w-[18rem]">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 sm:py-4 border-b border-border/50 flex-shrink-0">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="font-display text-sm tracking-wider text-primary" style={{ textShadow: "0 0 10px hsl(120 100% 54% / 0.5)" }}>LOGICIA</span>
            </Link>
            <button
          ...
          <div className="p-3 flex-shrink-0">
            ...
            >
              <Plus size={15} className="group-hover:rotate-90 transition-transform duration-300 flex-shrink-0" />
              NEW CONVERSATION
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-4">
            {conversations.length === 0 ? (
              <p className="text-center text-xs text-muted-foreground/50 mt-8 px-4">No conversations yet. Start one!</p>
            ) : (
              <>
                <p className="font-display text-[9px] tracking-[0.25em] text-muted-foreground/40 px-3 py-2">HISTORY</p>
                {conversations.map((conv) => (
                  ...
                  >
                    <span className="flex-1 text-xs truncate font-body">{conv.title}</span>
                    <button
          ...
          <div className="border-t border-border/50 p-4 flex-shrink-0">
            <Link to="/" className="flex items-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors group min-h-[36px]">
              <ArrowLeft size={13} className="group-hover:-translate-x-0.5 transition-transform flex-shrink-0" />
              Back to Home
            </Link>
          </div>
        </div>
      ...
      <div className="relative flex flex-col flex-1 min-w-0">
        ...
        {(!activeConv || activeConv.messages.length === 0) && (
          <div className="px-3 sm:px-6 pb-2 flex gap-2 overflow-x-auto flex-shrink-0" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="flex-shrink-0 border-t border-border/40 bg-background/80 backdrop-blur px-3 sm:px-6 py-3 sm:py-4">
          <div className="max-w-3xl mx-auto">
            <div className={`flex gap-2 items-end rounded-xl sm:rounded-2xl border bg-muted/10 px-3 sm:px-4 py-2.5 sm:py-3 transition-all duration-200 ${
              isOverLimit
              ...
              <div className="flex items-center gap-2 flex-shrink-0 pb-0.5">
                {showCounter && (
                  <span className={`text-[10px] font-mono tabular-nums ${isOverLimit ? "text-destructive" : "text-muted-foreground/50"}`}>{charsLeft}</span>
                )}
                <button
