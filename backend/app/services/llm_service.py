...
class LLMService:
    ...
    async def generate_response(self, user_query: str, math_context: Optional[Dict[str, Any]] = None, language: str = "en") -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "- MATH RENDERING RULE: You MUST use proper LaTeX for ALL mathematical expressions, formulas, and symbols. This ensures they are rendered perfectly on the user's screen.\n"
            "- Delimiters: Always wrap inline math in \\( ... \\) and block math in \\[ ... \\].\n"
            "    * Correct: \\(x^{2} + 5x + 6 = 0\\)\n"
            "    * Correct: \\[ \\int_{0}^{1} x \\, dx = \\frac{1}{2} \\]\n"
            "- Use LaTeX for powers, fractions, roots, Greek letters (\\theta, \\Omega, \\pi), and complex notation.\n"
            "- Avoid using plain text like 'x^2' or 'sqrt(x)' without LaTeX wrappers.\n"
            "- If a symbolic math result is provided by the engine, reference and explain it using the same LaTeX style.\n"
